-- Final Safe Database Migration for Harmony Spaces
-- This version handles existing event_categories table
...
-- Fix event_categories table - add missing columns if table exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'event_categories') THEN
    ...
    IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'event_categories_slug_key') THEN
      ALTER TABLE event_categories ADD CONSTRAINT event_categories_slug_key UNIQUE (slug);
    END IF;

    -- Add foreign key for parent_id if it doesn't exist
    IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'event_categories_parent_id_fkey') THEN
      ALTER TABLE event_categories ADD CONSTRAINT event_categories_parent_id_fkey
...
-- Only insert categories if slug column exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.columns
             WHERE table_name = 'event_categories' AND column_name = 'slug') THEN
  ...
  END IF;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not insert sample categories: %', SQLERRM;
END $$;

-- ========================================
-- COMPLETION MESSAGE
-- ========================================
-- Final safe migration completed!
-- This version handles existing event_categories table properly.
