...
DO $$ 
BEGIN
  -- Use pg_policy correctly with schemaname and tablename
  IF EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Space owners can manage animal types' 
    AND schemaname = 'public' 
    AND tablename = 'space_animal_types'
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Users can view space animal types' 
    AND schemaname = 'public' 
    AND tablename = 'space_animal_types'
  ) THEN
