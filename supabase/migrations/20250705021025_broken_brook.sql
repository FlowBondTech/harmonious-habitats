/*
  # Fix space animal types table and policies

  1. New Tables
    - Ensure `animals_allowed` column exists on spaces table
    - Ensure `space_animal_types` table exists with proper structure
  
  2. Security
    - Enable RLS on `space_animal_types` table
    - Add policies for authenticated users to manage and view animal types
*/
...
BEGIN
  IF EXISTS (
    ...
    WHERE polname = 'Space owners can manage animal types' AND tablename = 'space_animal_types'
  ) THEN
    DROP POLICY "Space owners can manage animal types" ON space_animal_types;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polname = 'Users can view space animal types' AND tablename = 'space_animal_types'
  ) THEN
    DROP POLICY "Users can view space animal types" ON space_animal_types;
  END IF;
END $$;
